...
function SignUpComponent({회원, isConfirmModalFn, isTimer, introMainFn}: any ){
    ...
    const onClickIdOkBtn=(e: MouseEvent<HTMLButtonElement>)=>{
        ...
        if( regExp2.test( thisVal )===false || regExp3.test(thisVal)===false || regExp4.test(thisVal)===true ){                        
            ...
            axios({ // CORS API
                url:'http://mosad.dothome.co.kr/react_cra/member_select.php',
                method:'GET'
            })
            .then((res)=>{
    ...
    //    전송버튼에서 
    //    타입스크립트에서 폼에서 전송하는 온서브밋 이벤트: onSubmit={}는 React.FormEvent로 설정
    const onSubmitEvent=(e: React.FormEvent)=>{
        ...
        if(state.아이디===''){
            ...
            // CORS REST API
            axios({
                url:'http://mosad.dothome.co.kr/react_cra/member_insert.php',
                method:'POST',
                data: newFormData
            })
            .then((res)=>{
